...
struct BlobView: View {
    ...
    var body: some View {
        Canvas { context, size in
            context.fill(myPath(in: rect), with: .linearGradient(
                Gradient.init(colors: [.pink.opacity(0.8), .blue.opacity(0.2)]), startPoint: CGPoint(x: 0, y: 0), endPoint: CGPoint(x: 400, y: 410)))
        }.frame(width: 400, height: 410)
    }
    
    func myPath(in rect: CGRect) -> Path {
        var path = Path()
